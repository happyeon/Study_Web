...
public class AnswerController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (command.equals("list")) {
			...
			if (biz.boardInsert(dto)) {
				jsResponse(response, "글 작성 성공", "answer.do?command=list");
			} else {
				jsResponse(response, "글 작성 실패", "answer.do?command=insertform");
			}
			...
			if(res) {
				jsResponse(response, "글 수정 성공", "answer.do?command=select&boardno="+boardno);
			} else {
			...
			if (biz.answerProc(dto)) {
				jsResponse(response, "답변 성공", "answer.do?command=list");
			} else {
				jsResponse(response, "답변 실패", "answer.do?answerform&boardno="+parentboardno);
			}
		}
	}
